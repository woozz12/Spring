					...
					<div class="form-group email-form">
						...
						<div class="input-group">
							<input type="text" name="userEmail1" class="form-control" id="userEmail1"
								placeholder="이메일"> <select name="userEmail2" class="form-control"
								id="userEmail2">
...
<script>
	...
	let idFlag, pwFlag; //정규 표현식 유효성 검사 여부 판단.
	...
	document.getElementById('idCheckBtn').onclick = function() {
		...
		*/
		...
		- Promise 객체를 자동으로 리턴하여 손쉽게 통신의 응답데이터를
		  ...
		  - 따라서, 서버가 응답한 정보 중 JSON 데이터만 소비하려면
		  json()이라는 메서드를 사용합니다.
		...
		*/
		...
		.then(res => {
			...
			//함수를 콜백 방식으로 전달합니다.
			//함수의 매개변수를 선언하면 해당 매개변수로 응답에 관련되
		...
		fetch('${pageContext.request.contextPath}/user/mailCheck?email=' + email)
			.then(res => res.text())
				.then(data => {
					...
					alert('인증번호가 전송되었습니다. 확인 후 입력란에 정확히 입력해 주세요.')
				}); //비동기 끝
	...
	document.querySelector('.mail-check-input').onblur = function(e) {
		...
		if(inputCode === code) {
			...
			e.target.focus(); //다시 입력할 수 잇도록 포커싱 주기.
		}
	} //인증번호 검증 끝.
        ...
        pw.onkeyup = function(){
            var regex = /^[A-Za-z0-9+]{8,16}$/;
             if(regex.test(document.getElementById("userPw").value )) {
                ...
                document.getElementById("msgPw").innerHTML = "부적절한 아이디입니다.";
				pwFlag = false;
            }
        }
        /*비밀번호 확인검사*/
        ...
        pwConfirm.onkeyup = function() {
            ...
            if(document.getElementById("pwConfirm").value == document.getElementById("userPw").value ) {
                ...
                document.getElementById("msgPw-c").innerHTML = "비밀번호 제대로 입력하세요";
            }
        }        
    </script>
